...
There is no analytics client, external AI integration, embedded secret, API token store, application server, or background online service. OSLog records build state and counts, not manuscript content. Full TeX logs can contain local paths and source excerpts and should be reviewed before sharing. Git network operations are explicit user actions and use the user's existing Git/SSH/Keychain configuration.
