...
`EditorCompletionController` debounces requests, rejects stale results after selection/source changes, and owns transient snippet stops. `CompletionPopup` is a nonactivating AppKit child panel containing a native table; the source editor retains keyboard focus. Tab/Return accepts one undoable replacement, arrows choose a row, and Escape dismisses. Tab advances through command arguments. Selection changes, scrolling, window changes, file switches, disabled completion, and editor teardown cancel the popup. No network request or TeX compilation is needed for suggestions.
