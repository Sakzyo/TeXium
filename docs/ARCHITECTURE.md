...
The line index adjusts offsets for the replaced range; only opening or replacing a whole buffer rebuilds it. Standard undo and NSTextFinder supply familiar editing. Switching tabs currently clears the view's undo stack; saved source and local history remain independent of that stack.

`CompletionProject` indexes symbols per source/bibliography file off the main actor. A completion request overlays current unsaved buffers, replacing each file's old symbols. `CompletionLexing` preserves UTF-16 offsets while excluding comments and literal environments, identifies the active command argument, and isolates the current key within comma-separated references. `LaTeXCompletion` ranks commands by prefix and project items by key prefix, substring, and contextual metadata. Existing arguments and token suffixes are preserved during replacement.

`EditorCompletionController` debounces requests, rejects stale results after selection/source changes, and owns a transient `CompletionSnippetSession`. The core session tracks UTF-16 field ranges, including selected defaults, as edits grow or shrink them. Tab/Shift-Tab selects the next/previous field, with a final stop after the insertion. Edits outside the fields cancel navigation. `CompletionEnvironments` preserves existing options and bodies, balances surrounding environment boundaries, and renders new structures using the editor's indentation and file line endings. Catalog-only field notation is expanded before native insertion.

`CompletionPopup` is a nonactivating AppKit child panel containing a native table; the source editor retains keyboard focus. Tab/Return accepts one undoable replacement, arrows choose a row, and Escape dismisses. Selection changes, scrolling, window changes, file switches, disabled completion, and editor teardown cancel the popup. No network request or TeX compilation is needed for suggestions.
